...
function errorMessage (er) {
  ...
  switch (er.code) {
    ...
    case 'EJSONPARSE':
      ...
      // Check whether we ran into a conflict in our own app.json
      if (er.file === path.join(npm.prefix, 'package.json')) {
        ...
        if (isDiff(txt)) {
          detail.push([
            '',
            [
              'Merge conflict detected in your app.json.',
              '',
              'Please resolve the app.json conflict and retry the command:',
              '',
              `$ ${process.argv.join(' ')}`
            ].join('\n')
          ])
      ...
      detail.push([
        'JSON.parse',
        [
          'Failed to parse app.json data.',
          'app.json must be actual JSON, not just JavaScript.'
        ].join('\n')
      ])
      break
    ...
    case 'EPUBLISHCONFLICT':
      ...
      detail.push(['publish fail', "Update the 'version' field in app.json and try again."])
    ...
    case 'ENOPACKAGEJSON':
      ...
      detail.push([
        'package.json',
        [
          "npm can't find a app.json file in your current directory."
        ].join('\n')
      ])
      break
