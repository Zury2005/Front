...
class FileSystem {
     ...
     */
    static CopyFiles(sourceFilePaths, destinationFilePaths) {
        if (!sourceFilePaths) {
        ...
        //
        const destinationDirectoryPaths = [...new Set(destinationFilePaths.map((filePath) => path.dirname(filePath)))]
            .sort()
     ...
     */
    static CopyNpmrcFile(settings, logger, destinationPath) {
        if (settings.UseLocalNpmrcFile) {
            ...
            if (fs.existsSync(npmrcFilePath)) {
                ...
                logger.Write(`Copied local NPM config file "${npmrcFilePath}" to staging directory "${destinationPath}".`, Verbosity_1.Verbosity.Debug);
            }
        }
    }
    /*
     ...
     */
    static CopyPackageDependencies(logger, packageName, installPath) {
        ...
        logger.Write(`Copying ${packageName} package dependencies to "${packageNodeModulesPath}".`, Verbosity_1.Verbosity.Debug);
        ...
        const destinationFilePaths = sourceFilePaths
            .map((filePath) => path.join(packageNodeModulesPath, filePath.split(installationNodeModulesPath).pop()));
        FileSystem.CopyFiles(sourceFilePaths, destinationFilePaths);
    }
    static EnumerateFilesRecursively(directoryPath) {
        let results = [];
     ...
     */
    static RemoveDirectoryRecursively(directoryPath) {
